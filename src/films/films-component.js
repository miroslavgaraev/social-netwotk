...
function FilmsComponent(props) {
    ...
    const {pageSize, totalFilmsCount, currentPage} = useSelector((state) => state.FilmsReducer)
    ...
    return (
        <div className='container'>
            <div className="input-group mb-3">
                ...
                <button onClick={searchFilms} className="btn btn-outline-secondary" type="button"
                        id="button-addon2">Поиск
                </button>
            </div>
            {search ?
                <select onChange={(e) => setFilterText(e.target.value)} id={'select'} className="form-select"
                    ...
                    return (
                        <div className="card">
                            ...
                            <div className="card-body">
                                ...
                                <NavLink to={`/films/${film.imdbID}`} className="btn btn-primary">Показать
                                    больше</NavLink>
                            </div>
                        </div>
                    )
            ...
            <nav aria-label="Page navigation example">
                <ul className="pagination">
                    ...
                    {pagesCount.slice(
                        ...
                        return(<li className="page-item"><button onClick={() => {searchFilms(page)}} className="page-link" >{page}</button></li>)
                    })}
